...
| 32 | 발표자 큐시트 (라이브 녹화용) | ✅ 완료 (2026-05-30) | `docs/demo/cue_sheet.md` — 7컷 × 3분 클릭·키·내레이션·타임라인, 준비 체크리스트, NG 모음. NotebookLM SCENE 1~7·스크린샷 팩과 정합 |
| 33 | 시연 대본 변형판 (1분·5분) | ✅ 완료 (2026-05-30) | `docs/demo/demo_1min.md`(학생·학부모 홍보 4 SCENE) + `demo_5min.md`(학과장·운영위 심화 9 SCENE — 공식·상담분류·거버넌스). 3분판과 사실·SCENE 구조 정합 |
| 34 | NotebookLM 전송·영상 생성 진행 순서 가이드 | ✅ 완료 (2026-05-30) | `docs/demo/notebooklm_howto.md` — 소스 추가→한국어→Video Overview→프롬프트→다운로드 6단계 + 청중별 대본 선택표 + 문제해결 체크리스트 |
| 35 | 워크스루 영상 2편 (실제 구동 화면 녹화) | ✅ 완료 (2026-05-30) | `assets/walkthrough_student.mp4`(STEP1→결과지→수강계획서, 키 실응답) + `walkthrough_admin.mp4`(대시보드·메뉴전환) + `tests/record_demo.mjs`. NotebookLM 개념영상이 앱 구동을 못 보여준다는 피드백 대응 (§22.5) |
| 36 | 워크스루 2편 → 1편 합본 | ✅ 완료 (2026-05-30) | `assets/walkthrough_full.mp4`(~42초, 1280×720) + `tests/combine_demo.mjs`. 한글 타이틀카드(브라우저 렌더)+공통 캔버스 패딩+concat. ffmpeg drawtext Windows 한글 인자 깨짐 회피 |
| — | Codex 컨텍스트(AGENTS.md)·스킬 커밋 + 프리뷰 시드 gitignore | ✅ 완료 (2026-05-30) | `AGENTS.md`(CLAUDE.md의 Codex 짝)·`.agents/skills/demo` 커밋, `.preview_seed.json` ignore. **유지보수: 명세 변경 시 CLAUDE.md↔AGENTS.md 동기화 필요** |
...
docs/demo/
  ...
  notebooklm_howto.md          ← NotebookLM 전송·영상 생성 단계별 진행 순서
  notebooklm_demo_3min.md      ← NotebookLM Video Overview용 3분 소스 (7 SCENE)
  assets/                      ← 스크린샷 팩 6장(PNG) + 워크스루 영상 2편(MP4) + 캡처/녹화법
  cue_sheet.md                 ← 발표자 큐시트 (라이브 녹화용 7컷)
  demo_1min.md · demo_5min.md  ← 1분 홍보판 · 5분 심화판 변형 대본
tests/capture_demo.mjs         ← 스크린샷 자동 캡처 (puppeteer-core + 시스템 Chrome headless)
tests/record_demo.mjs          ← 워크스루 영상 녹화 (프레임 캡처 + ffmpeg-static 인코딩)
tests/combine_demo.mjs         ← 두 영상 → 1편 합치기 (타이틀카드 + 1280×720 통일)
```
...
### 22.5 워크스루 영상 (실제 구동 MP4, `docs/demo/assets/*.mp4`)

NotebookLM 자동 영상은 **개념 설명**(내레이션 슬라이드)이라 앱 구동이 안 보인다. 실제
클릭·전환·응답이 보이는 **화면 녹화 영상**을 별도로 둔다(사용자 피드백, 2026-05-30).

- `walkthrough_full.mp4` — **합본**: 타이틀카드→학생→타이틀카드→관리자 (1280×720, ~42초). 카톡·발표용 기본.
- `walkthrough_student.mp4` — STEP1→2→3(키 1~5 실응답)→결과지→수강계획서 (412×892, ~23초)
- `walkthrough_admin.mp4` — 관리자 KPI·참여추이 + 사이드바 메뉴 전환 (1440×900, ~15초)
- 합치기: `node tests/combine_demo.mjs`. 해상도가 다른 두 편을 1280×720 흰 패딩 캔버스로 맞추고
  한글 타이틀카드(브라우저 렌더 PNG — ffmpeg drawtext는 Windows에서 한글 인자 깨짐)를 앞에 붙여 concat.
- **무음**. 완성형은 영상편집기(곰믹스/Clipchamp)에서 이 영상 위에 3분 대본 음성·자막을 얹는다.
- **재생성**: `npm i puppeteer-core ffmpeg-static --no-save`(반드시 한 줄 — 따로 설치 시 서로 prune)
  → `node tests/record_demo.mjs` (개별 `--student`/`--admin`). 페이스는 `hold()`·`scrollThrough()` 조정.
- **구현 메모**: puppeteer `page.screencast()`가 이 환경에서 빈 파일(0바이트)을 내므로,
  **뷰포트 프레임 캡처 + ffmpeg concat(이미지별 표시시간)** 방식으로 인코딩. 출력 경로에 한글·공백이
  있으면 ffmpeg가 실패하므로 임시 ASCII 경로에서 인코딩 후 `docs/demo/assets/`로 복사.
