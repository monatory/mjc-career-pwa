# 명지전문대학 학과추천 진단 PWA — Codex 작업 컨텍스트
...
다음 항목은 **개발자(=Codex) 임의로 수정 금지**. 사용자(진로취업팀) 명시 지시가 있어야만 변경 가능.
...
| 30 | 시연 자료 전용 폴더 + `demo` 스킬 구축 | ✅ 완료 (2026-05-30) | `docs/demo/`(매니페스트 README + 대본) 분리 + `.Codex/skills/demo/SKILL.md`. "시연 엠디"·"/demo" 호출 시 시연 레이어만 발동 (§22) |
...
> - 7) AGENTS.md 갱신(§3.8~3.10·§4.5~4.7·§5.4·§10 FAQ·§20) + `docs/STAGE_B_GUIDE.md`
> - 자세한 화면 흐름은 §20 참조.
...
| Analytics | **사용 안 함** (AGENTS.md §7 외부 분석 금지) |
...
**학번·이름 절대 저장 안 함**(AGENTS.md §7.2). 본 운영 전환 시 별도 `students/{studentId}` 컬렉션에 AES-256 암호화하여 보관하고, `responses`는 익명ID만 유지.
...
이 `AGENTS.md`는 **프로젝트의 살아있는 명세**. 다음과 같은 변경이 있으면 반드시 갱신:
...
문서가 코드보다 뒤처지면 다음 Codex 세션의 컨텍스트가 깨진다.
...
- 시연 자료의 수치·URL·기능 설명은 루트 `AGENTS.md`와 실제 코드를 진실 기준으로 삼는다
  (과장·미구현 "완성" 표기 금지).
