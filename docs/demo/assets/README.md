...
| 파일 | 내용 | 크기·길이 |
|---|---|---|
| **`walkthrough_full.mp4`** ⭐ | **두 편 합본** — 타이틀카드 → 학생 흐름 → 타이틀카드 → 관리자 (공통 1280×720 캔버스) | 1280×720, 약 42초 |
...
node tests/record_demo.mjs            # 두 영상(student·admin)
#   node tests/record_demo.mjs --student   / --admin   (개별)

# 5) 두 영상을 1편으로 합치기 → walkthrough_full.mp4 (타이틀카드 + 1280×720 통일)
node tests/combine_demo.mjs
```
